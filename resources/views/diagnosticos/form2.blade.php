<x-diagnosticos.layout :progressPct="8" progressLabel="Etapa 1b — Sobre o empreendimento">
    <div class="bg-white sm:rounded-lg shadow-sm border border-gray-100" style="padding: 35px;">
        <!-- Header -->
        <div class="mb-8">
            <span class="text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded-md" style="background-color: #fdf8ed; color: #D0AE6D;">Etapa 2 de 2</span>
        ...
        <form method="POST" action="{{ route('diagnostico.saveForm2', $token) }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nome_empreendimento" value="{{ old('nome_empreendimento', $diagnostico->nome_empreendimento) }}" required
                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#D0AE6D] focus:ring-[#D0AE6D] px-4 py-2.5 text-sm"
            ...
            <div class="bg-amber-50 border border-amber-200 sm:rounded-lg p-4">
                <label class="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" name="aceite" id="aceite" value="1"
                        class="mt-0.5 h-4 w-4 rounded border-gray-300 focus:ring-[#D0AE6D]"
                        style="accent-color: #D0AE6D;" required>
